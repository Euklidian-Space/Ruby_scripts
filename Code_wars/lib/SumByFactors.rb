...
def sum_of_divided arr 
  result = []
  prime_factors = arr.map{ |e| Prime.prime_division(e).map(&:first) }.flatten.uniq.select{ |e| e > 1 }.sort 
  prime_factors.each do |factor|
    result << [factor, (arr.select{ |n| n % factor == 0}).reduce(:+)]
  end
  result 
end